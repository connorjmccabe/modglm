...
# cfacs<-c(1,mean(pois$model$x1),quantile(pois$model$x2,.25),0)
# b0<-pois$coefficients["(Intercept)"]
# b1<-pois$coefficients["x1"]
# b2<-pois$coefficients["x2"]
# b3<-pois$coefficients["female"]
# b13<-pois$coefficients["x1:female"]
# x1<-mean(df$x1)
# x2.25<-quantile(df$x2,.25)
# x2.50<-quantile(df$x2,.50)
# x2.75<-quantile(df$x2,.75)
# 
# xi_1<-c(1,mean(df$x1),quantile(df$x2,.25),1,mean(df$x1))
# xi_0<-c(1,mean(df$x1),quantile(df$x2,.25),0,0)
# b<-pois$coefficients
# # 
# # b[1]*xi_1[1]+b[2]*xi_1[2]+b[3]*xi_1[3]+b[4]*xi_1[4]+b[5]*xi_1[5]
# # 
# # b0+b1*x1 + b2*x2.25 + b3*1 + b13*x1*1
# 
# (int.25.mat<-(b1+b13)*exp(xi_1%*%b) - b1*exp(xi_0%*%b))
# (int.25<-(b1+b13)*exp(b0+(b1+b13)*x1 + b3 + b2*x2.25) - b1*exp(b0+b1*x1+b2*x2.25))
# 
# (int.50<-(b1+b13)*exp(b0+(b1+b13)*x1 + b3 + b2*x2.50) - b1*exp(b0+b1*x1+b2*x2.50))
# (int.75<-(b1+b13)*exp(b0+(b1+b13)*x1 + b3 + b2*x2.75) - b1*exp(b0+b1*x1+b2*x2.75))
# 
# ######values for paper:
# (b1+b13)
# b1
...
# b0+(b1+b13)*x1 + b3
# 
# pois.ints<-modglm(model=pois,vars=c("x1","female"),data=df.test, dd=T)
# 
# cfacs25<-c(1,mean(pois$model$x1),quantile(pois$model$x2,.25),0)
# cfacs50<-c(1,mean(pois$model$x1),quantile(pois$model$x2,.50),0)
# cfacs75<-c(1,mean(pois$model$x1),quantile(pois$model$x2,.75),0)
# pois.ints25<-modglm(model=pois,vars=c("x1","female"),data=df, dd=T,cfacs=cfacs25)
# pois.ints50<-modglm(model=pois,vars=c("x1","female"),data=df, dd=T,cfacs=cfacs50)
# pois.ints75<-modglm(model=pois,vars=c("x1","female"),data=df, dd=T,cfacs=cfacs75)
# 
# pois.ints25$intcf
# pois.ints50$intcf
# pois.ints75$intcf
# 
# pois.ints25$intcf$int.est-1.96*pois.ints25$intcf$se.int.est
# pois.ints25$intcf$int.est+1.96*pois.ints25$intcf$se.int.est
# 
# pois.ints50$intcf$int.est-1.96*pois.ints50$intcf$se.int.est
# pois.ints50$intcf$int.est+1.96*pois.ints50$intcf$se.int.est
# 
# pois.ints75$intcf$int.est-1.96*pois.ints25$intcf$se.int.est
# pois.ints75$intcf$int.est+1.96*pois.ints25$intcf$se.int.est
# 
# pois.ints.x2q1<-modglm(model=pois,vars=c("x1","female"),data=df,cfacs = c(1,mean(pois$model$x1),quantile(pois$model$x2,.25),0), dd=T)
# pois.ints.x2q2<-modglm(model=pois,vars=c("x1","female"),data=df,cfacs = c(1,mean(pois$model$x1),quantile(pois$model$x2,.50),0), dd=T)
# pois.ints.x2q3<-modglm(model=pois,vars=c("x1","female"),data=df,cfacs = c(1,mean(pois$model$x1),quantile(pois$model$x2,.75),0), dd=T)
# 
# 0.65*exp(0.65*mean(pois$model$x1)+0.19)-0.33*exp(0.33*mean(pois$model$x1)-0.75)
# 0.65*exp(0.65*mean(pois$model$x1)+0.89)-0.33*exp(0.33*mean(pois$model$x1)-0.05)
...
sqrt(as.vector(colMeans(pois.ints$jac))%*%vcov(pois)%*%as.vector(t(colMeans(pois.ints$jac))))

dim(sqrt(pois.ints$jac%*%vcov(pois)%*%t(pois.ints$jac)))
